...
def forms_finder(text):
    ...
    verb_present_forms = set(re.findall('выпива(?:ю|ешь|ем|ет|ют)', text))
    verb_past_forms = set(re.findall('выпил[а-я]*', text))
    verb_participle_active = set(re.findall('выпивш[а-р,т-я]{2,3}', text))
    verb_participle_pass = set(re.findall('выпит[а-ы,э-я]{1,3}', text))
    verb_participle_transgressive = set(re.findall('выпив\s|выпимши', text))
    verb_reflexive_present_forms = set(re.findall('выпива(?:ю|ешь|ем|ет|ют)(?:ся|сь)', text))
    verb_reflexive_past_forms = set(re.findall('выпил[а-я]*(?:ся|сь)', text))
    verb_reflexive_participle_active = set(re.findall('выпивш[а-я]{2}(?:ся|сь)', text))
# похоже, возвратных форм пассивного залога не существует
# set() превращает список в беспорядочную коллекцию дистинктных обьетов, т е убирает повторения
    ...
    print('формы глагола в настоящем времени:',"\t".join(verb_present_forms))
    ...
    print('возвратные формы глагола в настоящем времени:',"\t".join(verb_reflexive_present_forms))
